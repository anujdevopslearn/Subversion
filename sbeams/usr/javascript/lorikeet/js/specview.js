...
(function($) {
    ...
    function plotPeakMassErrorPlot(container, datasets) {
        ...
        placeholder.append('<div id="' + getElementId(container, elementIds.massErrorPlot_unit) + '" class="link"  '
			   ...
			   + 'background-color:ffad4e; font-style:italic">'
	...
	if(options.sequence) {
	    ...
	    specinfo += '<div>';
	    specinfo += '<span style="font-weight:bold; color:#B00;">'+getModifiedSequence(options)+'</span>';
	...
	for(var i = 0; i < options.sequence.length; i += 1) {
	    ...
	    if(options.peptide.varMods()[i+1])
		modSeq += '<span style="background-color:ffad4e;padding:1px;border:1px dotted #CFCFCF;">'+options.sequence.charAt(i)+"</span>";
	    else

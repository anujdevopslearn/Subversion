...
var GraphicGenerator = Class.create( {
  ...
  generate: function() {
    ...
    $("errors").hide();
    var ids  = document.querySelectorAll('[id*="pfam_seq"]');
    for (let i = 1; i <= ids.length; i++) {
			if ( $("dg"+i).select("canvas").size() > 0 ) {
				$("dg"+i).select("canvas").first().remove();
			}
			var sequence;
			try {
				eval( "sequence = " + $("pfam_seq"+i).getValue() );
			} catch ( e ) {
				$("error").update( e );
				$("errors").show();
				return;
